...
//@Persistable
public class FieldMetadataImpl<T> extends MetadataDecorator<T, Metadata<T>> implements FieldMetadata<T>{
...
//    @Link(name = "field_data_model_", lazy = false, allowedTypes = { PersistentObjectMetadataImpl.class, MapDataModel.class,
...
//    protected Metadata<T> valueMetadata;
    ...
    public FieldMetadataImpl(Metadata<T> valueMetadata) {
	super();
	this.nestedMetadata = valueMetadata;
    }


    public FieldMetadataImpl(String name, Metadata<T> valueMetadata) {
	super();
	this.name = name;
	this.nestedMetadata = valueMetadata;
    }
...
//    }
	...
	if (isRequired() && value == null) {
	    throw new ValidationException(ValidationErrorType.REQUIRED, new GenericNameValue(getName(), value));
	}
	return true;
    }
	...
	return "FieldMetadataImpl [name=" + name + ", required=" + required + ", implementation=" + implementation
